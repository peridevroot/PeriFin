...
contract CollateralManager is ICollateralManager, Owned, Pausable, MixinResolver {
    ...
    // The maximum amount of debt in pUSD that can be issued by non snx collateral.
    uint public maxDebt;
    ...
    function totalLong() public view returns (uint susdValue, bool anyRateIsInvalid) {
        ...
        if (pynths.length > 0) {
            for (uint i = 0; i < pynths.length; i++) {
                ...
                if (pynth == pUSD) {
                    susdValue = susdValue.add(state.long(pynth));
                } else {
    ...
    function getBorrowRate() external view returns (uint borrowRate, bool anyRateIsInvalid) {
        ...
        uint snxDebt = _issuer().totalIssuedPynths(pUSD, true);
