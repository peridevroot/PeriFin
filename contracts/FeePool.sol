...
contract FeePool is Owned, Proxyable, LimitedSetup, MixinSystemSettings, IFeePool {
    ...
    bytes32 private constant CONTRACT_ETH_COLLATERAL_SUSD = "EtherCollateralpUSD";
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        newAddresses[6] = CONTRACT_PYNTHETIXSTATE;
    ...
    function etherCollateralpUSD() internal view returns (IEtherCollateralpUSD) {
        return IEtherCollateralpUSD(requireAndGetAddress(CONTRACT_ETH_COLLATERAL_SUSD));
    }
     ...
     * @param debtEntryIndex The index in the global debt ledger. perifinState.issuanceData(account)
     * @dev onlyIssuer to call me on perifin.issue() & perifin.burn() calls to store the locked PERI
    ...
    function closeCurrentFeePeriod() external issuanceActive {
        ...
        _recentFeePeriodsStorage(0).startingDebtIndex = uint64(perifinState().debtLedgerLength());
        _recentFeePeriodsStorage(0).startTime = uint64(now);
    ...
    function importFeePeriod(
        ...
        require(startingDebtIndex <= perifinState().debtLedgerLength(), "Cannot import bad data");
     ...
     * @param snxAmount The amount of PERI tokens.
     */
     ...
     * @param account The address to send the fees to.
     * @param snxAmount The amount of PERI.
     */
    ...
    function _effectiveDebtRatioForPeriod(
        ...
        // This is a high precision integer.
        IPeriFinState _perifinState = perifinState();
        uint feePeriodDebtOwnership = _perifinState
            .debtLedger(closingDebtIndex)
            .divideDecimalRoundPrecise(_perifinState.debtLedger(debtEntryIndex))
    ...
    event FeesClaimed(address account, uint pUSDAmount, uint snxRewards);
    ...
    function emitFeesClaimed(
        ...
        proxy._emit(abi.encode(account, pUSDAmount, snxRewards), 1, FEESCLAIMED_SIG, 0, 0, 0);
    }
}
