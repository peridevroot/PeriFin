...
contract MintablePeriFin is BasePeriFin {
    ...
    constructor(
        ...
        uint _totalSupply,
        address _resolver
    ) public BasePeriFin(_proxy, _tokenState, _owner, _totalSupply, _resolver) {}
    ...
    function onlyAllowFromBridge() internal view {
        require(msg.sender == perifinBridge(), "Can only be invoked by bridge");
    }
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        bytes32[] memory existingAddresses = BasePeriFin.resolverAddressesRequired();
        bytes32[] memory newAddresses = new bytes32[](1);
        newAddresses[0] = CONTRACT_PYNTHETIX_BRIDGE;
        addresses = combineArrays(existingAddresses, newAddresses);
    }

    function perifinBridge() internal view returns (address) {
        return requireAndGetAddress(CONTRACT_PYNTHETIX_BRIDGE);
    }
