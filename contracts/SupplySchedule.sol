...
contract SupplySchedule is Owned, ISupplySchedule {
    ...
    // The number of PERI rewarded to the caller of PeriFin.mint()
    ...
    // Address of the PeriFinProxy for the onlyPeriFin modifier
    address payable public perifinProxy;
    ...
    function mintableSupply() external view returns (uint) {
        ...
        while (remainingWeeksToMint > 0) {
            ...
            if (currentWeek < SUPPLY_DECAY_START) {
                ...
                // Terminal supply is calculated on the total supply of PeriFin including any new supply
                // We can compound the remaining week's supply at the fixed terminal rate
                uint totalSupply = IERC20(perifinProxy).totalSupply();
    ...
    /**
     * @notice Record the mint event from PeriFin by incrementing the inflation
     * week counter for the number of weeks minted (probabaly always 1)
     * and store the time of the event.
     * @param supplyMinted the amount of PERI the total supply was inflated by.
     * */
    function recordMintEvent(uint supplyMinted) external onlyPeriFin returns (bool) {
    ...
    /**
     * @notice Set the PeriFinProxy should it ever change.
     * SupplySchedule requires PeriFin address as it has the authority
     * to record mint event.
     * */
    function setPeriFinProxy(IPeriFin _perifinProxy) external onlyOwner {
        require(address(_perifinProxy) != address(0), "Address cannot be 0");
        perifinProxy = address(uint160(address(_perifinProxy)));
        emit PeriFinProxyUpdated(perifinProxy);
    }

    // ========== MODIFIERS ==========

    /**
     * @notice Only the PeriFin contract is authorised to call this function
     * */
    modifier onlyPeriFin() {
        require(
            msg.sender == address(Proxy(address(perifinProxy)).target()),
            "Only the perifin contract can perform this action"
        );
        _;
    }
    ...
    /**
     * @notice Emitted when setPeriFinProxy is called changing the PeriFin Proxy address
     * */
    event PeriFinProxyUpdated(address newAddress);
}
