...
// https://docs.perifin.io/contracts/source/contracts/depot
contract Depot is Owned, Pausable, ReentrancyGuard, MixinResolver, IDepot {
    ...
    bytes32 private constant CONTRACT_PYNTHSUSD = "PynthpUSD";
        ...
        for (uint i = depositStartIndex; remainingToFulfill > 0 && i < depositEndIndex; i++) {
            ...
            if (deposit.user == address(0)) {
                ...
                if (deposit.amount > remainingToFulfill) {
                    ...
                    // perifin foundation funds wallet. This is to protect all depositors
    ...
    function _exchangeEtherForPERI() internal returns (uint) {
        ...
        uint perifinToSend = perifinReceivedForEther(msg.value);

        // Store the ETH in our funds wallet
        fundsWallet.transfer(msg.value);

        // And send them the PERI.
        perifin().transfer(msg.sender, perifinToSend);

        emit Exchange("ETH", msg.value, "PERI", perifinToSend);

        return perifinToSend;
    }
     ...
     * @param guaranteedPeriFinRate The perifin exchange rate which must be honored or the call will revert.
     */
    function exchangeEtherForPERIAtRate(uint guaranteedEtherRate, uint guaranteedPeriFinRate)
        external
        payable
        rateNotInvalid(PERI)
        rateNotInvalid(ETH)
        ...
        )
    {
        ...
        require(
            guaranteedPeriFinRate == exchangeRates().rateForCurrency(PERI),
            "Guaranteed perifin rate would not be received"
        );
    ...
    function _exchangePynthsForPERI(uint pynthAmount) internal returns (uint) {
        // How many PERI are they going to be receiving?
        uint perifinToSend = perifinReceivedForPynths(pynthAmount);
        ...
        emit Exchange("pUSD", pynthAmount, "PERI", perifinToSend);
     ...
     *         exchange while protecting against frontrunning by the contract owner on the exchange rate.
     * @param pynthAmount The amount of pynths the user wishes to exchange.
     * @param guaranteedRate A rate (perifin price) the caller wishes to insist upon.
     */
     ...
     */
    function withdrawPeriFin(uint amount) external onlyOwner {
        perifin().transfer(owner, amount);

        // We don't emit our own events here because we assume that anyone
        // who wants to watch what the Depot is doing can
        // just watch ERC20 events from the Pynth and/or PeriFin contracts
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        addresses[2] = CONTRACT_PYNTHETIX;
    }
     ...
     */
    function perifinReceivedForPynths(uint amount) public view returns (uint) {
     ...
     */
    function perifinReceivedForEther(uint amount) public view returns (uint) {
        ...
        return perifinReceivedForPynths(valueSentInPynths);
    }
    ...
    function pynthpUSD() internal view returns (IERC20) {
        return IERC20(requireAndGetAddress(CONTRACT_PYNTHSUSD));
    }

    function perifin() internal view returns (IERC20) {
        return IERC20(requireAndGetAddress(CONTRACT_PYNTHETIX));
    }
