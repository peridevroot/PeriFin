...
// Used to have strongly-typed access to internal mutative functions in PeriFin
interface IPeriFinInternal {
...
contract Exchanger is Owned, MixinSystemSettings, IExchanger {
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        newAddresses[3] = CONTRACT_PYNTHETIX;
    ...
    ) external onlyPeriFinorPynth returns (uint amountReceived) {
        uint fee;
    ...
    function exchangeWithVirtual(
        ...
        address destinationAddress,
        bytes32 trackingCode
    ) external onlyPeriFinorPynth returns (uint amountReceived, IVirtualPynth vPynth) {
        uint fee;
    ...
    function _emitTrackingEvent(
        ...
        IPeriFinInternal(address(perifin())).emitExchangeTracking(trackingCode, toCurrencyKey, toAmount);
    }
    ...
    function _updatePERIIssuedDebtOnExchange(bytes32[2] memory currencyKeys, uint[2] memory currencyRates) internal {
        bool includesSUSD = currencyKeys[0] == pUSD || currencyKeys[1] == pUSD;
        ...
        )
    {
        ...
        IPeriFinInternal(address(perifin())).emitPynthExchange(
            from,
            sourceCurrencyKey,
            sourceAmountAfterSettlement,
            destinationCurrencyKey,
    ...
    function reclaim(
        ...
        IPeriFinInternal(address(perifin())).emitExchangeReclaim(from, currencyKey, amount);
    }
    ...
    function refund(
        ...
        IPeriFinInternal(address(perifin())).emitExchangeRebate(from, currencyKey, amount);
    }
    ...
    modifier onlyPeriFinorPynth() {
        IPeriFin _perifin = perifin();
        require(
            msg.sender == address(_perifin) || _perifin.pynthsByAddress(msg.sender) != bytes32(0),
            "Exchanger: Only perifin or a pynth contract can perform this action"
        );
        _;
    }
