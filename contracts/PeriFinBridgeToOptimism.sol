...
contract PeriFinBridgeToOptimism is Owned, MixinSystemSettings, IPeriFinBridgeToOptimism {
    ...
    bytes32 private constant CONTRACT_OVM_PYNTHETIXBRIDGETOBASE = "ovm:PeriFinBridgeToBase";
    ...
    function perifinERC20() internal view returns (IERC20) {
    ...
    function perifinBridgeToBase() internal view returns (address) {
        return requireAndGetAddress(CONTRACT_OVM_PYNTHETIXBRIDGETOBASE);
    }
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        newAddresses[4] = CONTRACT_OVM_PYNTHETIXBRIDGETOBASE;
    ...
    function initiateRewardDeposit(uint amount) external requireActive {
        ...
        perifinERC20().transferFrom(msg.sender, address(this), amount);
    ...
    function completeWithdrawal(address account, uint256 amount) external requireActive {
        ...
        require(messenger().xDomainMessageSender() == perifinBridgeToBase(), "Only the L2 bridge can invoke");

        // transfer amount back to user
        perifinERC20().transfer(account, amount);
    ...
    function migrateBridge(address newBridge) external onlyOwner requireActive {
        ...
        IERC20 ERC20PeriFin = perifinERC20();
        // get the current contract balance and transfer it to the new PeriFinL1ToL2Bridge contract
        uint256 contractBalance = ERC20PeriFin.balanceOf(address(this));
        ERC20PeriFin.transfer(newBridge, contractBalance);
    ...
    function _initiateRewardDeposit(uint256 _amount) internal {
        ...
        IPeriFinBridgeToBase bridgeToBase;
    ...
    function _initiateDeposit(uint256 _depositAmount) private {
        ...
        perifinERC20().transferFrom(msg.sender, address(this), _depositAmount);
        // create message payload for L2
        IPeriFinBridgeToBase bridgeToBase;
