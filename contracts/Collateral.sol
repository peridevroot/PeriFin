...
contract Collateral is ICollateralLoan, Owned, MixinSystemSettings {
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        newAddresses[4] = CONTRACT_PYNTHSUSD;
    ...
    function openInternal(
        ...
        // 2. We can only issue certain pynths.
        require(pynthsByKey[currency] > 0, "Not allowed to issue this pynth");

        // 3. Make sure the pynth rate is not invalid.
        ...
        // 15. If its short, convert back to pUSD, otherwise issue the loan.
        if (short) {
    ...
    function liquidateInternal(
        ...
        // 15. Burn the pynths from the liquidator.
    ...
    function repayInternal(
        ...
        // 6. Check the spender has enough pynths to make the repayment
    ...
    function drawInternal(uint id, uint amount) internal {
        ...
        // 9. If its short, let the child handle it, otherwise issue the pynths.
        if (loan.short) {
    ...
    function accrueInterest(Loan memory loan) internal returns (Loan memory loanAfter) {
        ...
        (uint rate, bool invalid) = loan.short
            ? _manager().getShortRate(pynthsByKey[loan.currency])
