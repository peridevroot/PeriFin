...
contract Liquidations is Owned, MixinSystemSettings, ILiquidations {
    ...
    // refactor to perifin storage eternal storage contract once that's ready
    ...
    function isOpenForLiquidation(address account) external view returns (bool) {
        uint accountCollateralisationRatio = perifin().collateralisationRatio(account);
    ...
    function flagAccountForLiquidation(address account) external rateNotInvalid("PERI") {
        ...
        uint accountsCollateralisationRatio = perifin().collateralisationRatio(account);
