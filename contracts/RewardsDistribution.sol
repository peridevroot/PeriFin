...
contract RewardsDistribution is Owned, IRewardsDistribution {
    ...
    /**
     * @notice Address of the PeriFin ProxyERC20
     */
    address public perifinProxy;
    ...
    /**
     * @dev _authority maybe the underlying perifin contract.
     * Remember to set the authority on a perifin upgrade
     */
    constructor(
        address _owner,
        address _authority,
        address _perifinProxy,
        address _rewardEscrow,
        address _feePoolProxy
    ) public Owned(_owner) {
        authority = _authority;
        perifinProxy = _perifinProxy;
    ...
    function setPeriFinProxy(address _perifinProxy) external onlyOwner {
        perifinProxy = _perifinProxy;
    }
    ...
    function distributeRewards(uint amount) external returns (bool) {
        ...
        require(perifinProxy != address(0), "PeriFinProxy is not set");
        require(feePoolProxy != address(0), "FeePoolProxy is not set");
        require(
            IERC20(perifinProxy).balanceOf(address(this)) >= amount,
        ...
        for (uint i = 0; i < distributions.length; i++) {
            if (distributions[i].destination != address(0) || distributions[i].amount != 0) {
                ...
                // Transfer the PERI
                IERC20(perifinProxy).transfer(distributions[i].destination, distributions[i].amount);
        ...
        IERC20(perifinProxy).transfer(rewardEscrow, remainder);
