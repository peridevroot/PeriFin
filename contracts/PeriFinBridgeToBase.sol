...
import "./interfaces/IPeriFinBridgeToBase.sol";

// Internal references
import "./interfaces/IPeriFin.sol";
import "./interfaces/IRewardEscrowV2.sol";
import "./interfaces/IPeriFinBridgeToOptimism.sol";

// solhint-disable indent
import "@eth-optimism/contracts/build/contracts/iOVM/bridge/iOVM_BaseCrossDomainMessenger.sol";


contract PeriFinBridgeToBase is Owned, MixinSystemSettings, IPeriFinBridgeToBase {
    ...
    bytes32 private constant CONTRACT_BASE_PYNTHETIXBRIDGETOOPTIMISM = "base:PeriFinBridgeToOptimism";
    ...
    function perifinBridgeToOptimism() internal view returns (address) {
        return requireAndGetAddress(CONTRACT_BASE_PYNTHETIXBRIDGETOOPTIMISM);
    }
    ...
    function onlyAllowFromOptimism() internal view {
        ...
        require(_messenger.xDomainMessageSender() == perifinBridgeToOptimism(), "Only the L1 bridge can invoke");
    }
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        newAddresses[1] = CONTRACT_PYNTHETIX;
        newAddresses[2] = CONTRACT_BASE_PYNTHETIXBRIDGETOOPTIMISM;
    ...
    function initiateWithdrawal(uint amount) external {
        require(perifin().transferablePeriFin(msg.sender) >= amount, "Not enough transferable PERI");

        // instruct L2 PeriFin to burn this supply
        perifin().burnSecondary(msg.sender, amount);

        // create message payload for L1
        IPeriFinBridgeToOptimism bridgeToOptimism;
    ...
    function completeEscrowMigration(
        ...
        perifin().mintSecondary(address(rewardEscrow), escrowedAmount);
    ...
    function completeDeposit(address account, uint256 depositAmount) external onlyOptimismBridge {
        // now tell PeriFin to mint these tokens, deposited in L1, into the same account for L2
        perifin().mintSecondary(account, depositAmount);
        emit MintedSecondary(account, depositAmount);
    }

    // invoked by Messenger on L2
    function completeRewardDeposit(uint256 amount) external onlyOptimismBridge {
        // now tell PeriFin to mint these tokens, deposited in L1, into reward escrow on L2
        perifin().mintSecondaryRewards(amount);
