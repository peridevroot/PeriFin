...
contract EtherCollateralpUSD is Owned, Pausable, ReentrancyGuard, MixinResolver, IEtherCollateralpUSD {
    ...
    address internal constant FEE_ADDRESS = 0xc8d3A942a82290612DD8167Bd9ce2d3054BAf8B7;
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        addresses[1] = CONTRACT_PYNTHSUSD;
    ...
    function currentInterestOnLoan(address _account, uint256 _loanID) external view returns (uint256) {
        ...
        uint256 currentInterest = accruedInterestOnLoan(
            pynthLoan.loanAmount.add(pynthLoan.accruedInterest),
            _timeSinceInterestAccrual(pynthLoan)
        );
        ...
        returns (uint256 loanID)
    {
        ...
        PynthLoanStruct memory pynthLoan = PynthLoanStruct({
            account: msg.sender,
    ...
    function _timeSinceInterestAccrual(PynthLoanStruct memory _pynthLoan) private view returns (uint256 timeSinceAccrual) {
        ...
        uint256 lastInterestAccrual = _pynthLoan.lastInterestAccrued > 0
            ? uint256(_pynthLoan.lastInterestAccrued)
            : _pynthLoan.timeCreated;
