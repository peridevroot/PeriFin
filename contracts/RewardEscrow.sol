...
contract RewardEscrow is Owned, IRewardEscrow {
    ...
    /* The corresponding PeriFin contract. */
    IPeriFin public perifin;
    ...
    /* An account's total escrowed perifin balance to save recomputing this for fee extraction purposes. */
    mapping(address => uint) public totalEscrowedAccountBalance;

    /* An account's total vested reward perifin. */
    mapping(address => uint) public totalVestedAccountBalance;

    /* The total remaining escrowed balance, for verifying the actual perifin balance of this contract against. */
    ...
    /**
     * @notice set the perifin contract address as we need to transfer PERI when the user vests
     */
    function setPeriFin(IPeriFin _perifin) external onlyOwner {
        perifin = _perifin;
        emit PeriFinUpdated(address(_perifin));
    }
     ...
     * @return A pair of uints: (timestamp, perifin quantity).
     */
     ...
     * @dev A call to this should accompany a previous successful call to perifin.transfer(rewardEscrow, amount),
    ...
    function vest() external {
        ...
        if (total != 0) {
            ...
            IERC20(address(perifin)).transfer(msg.sender, total);
    ...
    event PeriFinUpdated(address newPeriFin);
