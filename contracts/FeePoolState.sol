...
// https://docs.perifin.io/contracts/source/contracts/feepoolstate
contract FeePoolState is Owned, LimitedSetup {
     ...
     * @param debtEntryIndex The index in the global debt ledger. perifin.perifinState().issuanceData(account)
     * @param currentPeriodStartDebtIndex The startingDebtIndex of the current fee period
     * @dev onlyFeePool to call me on perifin.issue() & perifin.burn() calls to store the locked PERI
    ...
    /**
     * @notice Import issuer data from perifinState.issuerData on FeePeriodClose() block #
